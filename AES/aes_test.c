...
{    
    ...
    EVP_CIPHER_CTX *ctx_enc = encrypt_init_AES128(key, iv);
    for(int i=0; i<4; i++){
        //fprintf(stderr, "Step %d: Total length = %d\n", i, total_len_enc);
        encrypt_block_AES128(ctx_enc, key, iv, &plaintext[i*16], 16, &ciphertext_block[i*16], &enc_length);
    ...
    EVP_CIPHER_CTX *ctx_dec = decrypt_init_AES128(key, iv);
    for(int i=0; i<4; i++){
        //fprintf(stderr, "Step %d: Total length = %d\n", i, total_len_dec);
        decrypt_block_AES128(ctx_dec, key, iv, ciphertext_block+i*16, 16, decryptedtext_block+i*16, &dec_length);
