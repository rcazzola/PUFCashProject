...
// ONLY 500 characters allocated for these strings above.
         ...
         if ( SAP_ptr->ChipScalingConstantArr != NULL )
            printf("\tSKE SUCCESSFUL AUTHENTICATION: Chip %3d\tPersonalized ScalingConstant %f (Scaling? %d)\tID %s\tPCC %f\tIS >\tThreshold %f!\n", 
               SAP_ptr->chip_num, SAP_ptr->ChipScalingConstantArr[SAP_ptr->chip_num], do_scaling, ID_str, AE_PCC, PCC_SKE_AUTHEN_THRESHOLD);
         else
            printf("\tSKE SUCCESSFUL AUTHENTICATION: Chip %3d\tID %s\tPCC %f\tIS >\tThreshold %f!\n", SAP_ptr->chip_num, ID_str, 
               AE_PCC, PCC_SKE_AUTHEN_THRESHOLD);
         fflush(stdout);
...
int KEK_VerifierAuthentication(int max_string_len, SRFAlgoParamsStruct *SAP_ptr, int device_socket_desc, 
   int RANDOM)
   {
   int set_threshold_to_zero, target_attempts, do_part_A_part_B_both;

   char request_str[max_string_len];

   int enroll_or_regen, bits_remaining, SHD_num_bytes;
//   int enroll_num_SBS_bits;

   unsigned char *KEK_authentication_nonce;
   int num_strong_bits_encoded; 
   int dummy1, dummy2; 

   unsigned char *KEK_authen_XMR_SHD;
   int current_XMR_SHD_num_bytes; 
   char current_XMR_SHD_num_bytes_str[max_string_len]; 
...
// Store the SpreadFactors in a separate array for re-use below.
   signed char *SpreadFactors_binary = NULL;

   printf("\t\t\t******************* VERIFIER AUTHENTICATION BEGINS  ******************* \n\n"); fflush(stdout);

   current_function = FUNC_VA;

// Here we know which device has authenticated and we are doing the inverse, server authentication. The server computes the PO/PCR/PBD SF and
// transmits to device. Setting this to ONE ensures the device gets the FLIPPED PopOnly SF computed in the ComputePxxSpreadFactors routine 
// for ANY mode. Here, when 1, the server needs to compute the SF COMPLETELY (device does NOT do it), in which case, the PopOnly SF MUST be 
// updated to flip the distribution on the device and then PCR are computed later (after the call to generate the Pop SF).
   SAP_ptr->do_PO_dist_flip = 1;

// Set the 'normal' XMR value
   SAP_ptr->XMR_val = XMR_VAL;
   SAP_ptr->param_RangeConstant = RANGE_CONSTANT; 

// Sanity check
   if ( (SAP_ptr->num_KEK_authen_nonce_bits % 8) != 0 )
      { printf("ERROR: KEK_VerifierAuthentication(): SAP_ptr->num_KEK_authen_nonce_bits MUST be divisible by 8!\n"); exit(EXIT_FAILURE); }
   if ( (KEK_authentication_nonce = (unsigned char *)malloc(sizeof(unsigned char) * SAP_ptr->num_KEK_authen_nonce_bits/8)) == NULL )
      { printf("ERROR: KEK_VerifierAuthentication(): Failed to allocate storage for KEK_authentication_nonce!\n"); exit(EXIT_FAILURE); }

// Do only the first part of CommonCore, which selects the challenges, collects timing data for those challenges, sends the vectors to the
// device and does the XOR_nonce exchange. NOTE: We MUST DO THIS before attempting to get the KEK_authentication_nonce because the device
// runs 'CollectPNs' which generates nonces that are used for the KEK_authentication_nonce retrieved from the device below. Actually, with
// the TRNG on the device now, this is not necessary.
   set_threshold_to_zero = 0;
   do_part_A_part_B_both = 0;
...
// Do NOT compute PCR (or PBD) SF (which is irrelevant because compute_SpreadFactors is 0).
   ...
   target_attempts = 0;
   CommonCore(max_string_len, SAP_ptr, device_socket_desc, RANDOM, set_threshold_to_zero, target_attempts, do_part_A_part_B_both, current_function, 
      compute_SpreadFactors, send_SpreadFactors, compute_PCR_PBD_SF);

// Get KEK_authentication nonce from device. NOTE: We must wait for the CollectPNs to finish generating the random nonce bytes before receiving these.
   if ( SockGetB(SAP_ptr->KEK_authentication_nonce, SAP_ptr->num_KEK_authen_nonce_bits/8, device_socket_desc) != SAP_ptr->num_KEK_authen_nonce_bits/8 )
      { printf("KEK_VerifierAuthentication(): KEK_authentication_nonce receive failed!\n"); exit(EXIT_FAILURE); }

// Create local copies of authentication nonce information.
   bits_remaining = SAP_ptr->num_KEK_authen_nonce_bits; 

#ifdef DEBUG
printf("KEK_VerifierAuthentication(): Received %d KEK_authentication nonce bits from device\n", SAP_ptr->num_KEK_authen_nonce_bits); fflush(stdout);
#endif

// Need to make a copy of this locally because it is destroyed below.
   for ( i = 0; i < SAP_ptr->num_KEK_authen_nonce_bits/8; i++ )
      KEK_authentication_nonce[i] = SAP_ptr->KEK_authentication_nonce[i];

// Loop incrementing LFSR seed until we succeed in generating XMR_SHD that completely encodes the device's authentication nonce. 
// We need to iterate here until device indicates that it has enough bits.
...
#ifdef DEBUG
printf("KEK_VerifierAuthentication(): Starting iteration %d!\n", target_attempts); fflush(stdout);
#endif

// Here we only select parameters, set the LFSR_seed_high and compute SpreadFactors. This calculation is done here to enable server can calculate XMR_SHD 
// so ONLY compute SpreadFactors and DO NOT SEND TO DEVICE -- not needed yet -- done below. Server computes PCR/PBD SF.
      set_threshold_to_zero = 0;
      do_part_A_part_B_both = 1;
...
// Call ComputePxxSpreadFactors a second time but only if SF mode is NOT PopOnly. 
      if ( SAP_ptr->param_PCR_or_PBD_or_PO == SF_MODE_PCR || SAP_ptr->param_PCR_or_PBD_or_PO == SF_MODE_PBD )
         compute_PCR_PBD_SF = 1;
      else
         compute_PCR_PBD_SF = 0;

      CommonCore(max_string_len, SAP_ptr, device_socket_desc, RANDOM, set_threshold_to_zero, target_attempts, do_part_A_part_B_both, current_function, 
         compute_SpreadFactors, send_SpreadFactors, compute_PCR_PBD_SF);

// 1_1_2022: BUG: Forgot to do this here when mode is PopOnly! Since we do NOT run the PCR part of ComputePxxSpreadFactors(), we do NOT run DoSRFComp
// but we must run it before running SingleHelpBitGen below. 
      if ( SAP_ptr->param_PCR_or_PBD_or_PO == SF_MODE_POPONLY )
         DoSRFComp(max_string_len, SAP_ptr, 0);

      if ( (SpreadFactors_binary = (signed char *)realloc(SpreadFactors_binary, ((target_attempts+1) * SAP_ptr->num_SF_words) * sizeof(signed char))) == NULL )
         { printf("ERROR: KEK_VerifierAuthentication(): Failed to allocate storage for SpreadFactors_binary!\n"); exit(EXIT_FAILURE); }
      for ( i = 0; i < SAP_ptr->num_SF_words; i++ )
         SpreadFactors_binary[target_attempts*SAP_ptr->num_SF_words + i] = SAP_ptr->iSpreadFactors[i];

#ifdef DEBUG
for ( i = 0; i < 20; i++ )
   printf("KEK_VerifierAuthentication(): %d) fSF %9.4f\tiSF %4d\n", i, SAP_ptr->fSpreadFactors[i], SAP_ptr->iSpreadFactors[i]);
printf("\n");
fflush(stdout);
PrintHeaderAndHexVals("Server Computed SpreadFactors:\n", SAP_ptr->num_SF_bytes, (unsigned char *)SAP_ptr->iSpreadFactors, 32);
#endif

// Run the SingleHelpBitGen algorithm to compute the the SBS and SHD for the KEK_FSB_SKE routine below.
//      enroll_num_SBS_bits = SingleHelpBitGen(SAP_ptr->num_required_PNDiffs, SAP_ptr->fPNDco, SAP_ptr->device_SBS, SAP_ptr->device_SHD, 
      SingleHelpBitGen(SAP_ptr->num_required_PNDiffs, SAP_ptr->fPNDco, SAP_ptr->device_SBS, SAP_ptr->device_SHD, 
         &SHD_num_bytes, SAP_ptr->param_Threshold);

#ifdef DEBUG
printf("KEK_VerifierAuthentication(): SBS bits %d\n", enroll_num_SBS_bits); fflush(stdout);
#endif

// KEK SKE Enroll. Here, the PRODUCT is KEK_authen_XMR_SHD_chunk, the helper data bitstring that encodes the KEK_authentication_nonce using device_SHD and 
// device_SBS data. We ask on each iteration that it encode a portion of the KEK_authentication_nonce. It returns the number of bits from KEK_authentication_nonce 
// it was able to encode. We need to advance past those on each consecutive call. 
      int do_mismatch_count = 0;
      int FSB_or_SKE = 1;
      num_strong_bits_encoded = KEK_FSB_SKE(SAP_ptr->num_required_PNDiffs, SAP_ptr->XMR_val, SAP_ptr->device_SHD, SAP_ptr->device_SBS, 
         SAP_ptr->KEK_authen_XMR_SHD_chunk, bits_remaining, KEK_authentication_nonce, enroll_or_regen, do_mismatch_count, &dummy1, 
            0, NULL, &dummy2, FSB_or_SKE, SAP_ptr->chip_num, 0);

#ifdef DEBUG
printf("KEK_VerifierAuthentication(): Number of authentication bits encoded %d\n", num_strong_bits_encoded); fflush(stdout);
#endif

// Now we need to eliminate 'num_strong_bits_encoded' from KEK_authentication_nonce and move all the remaining bits to start at the first bit/byte. 
      if ( num_strong_bits_encoded > 0 )
         bits_remaining = EliminatePackedBitsFromBS(bits_remaining, KEK_authentication_nonce, num_strong_bits_encoded);

#ifdef DEBUG
printf("\tIteration %d\tbits remaining in KEK_authentication_nonce to encode %d\n\n", target_attempts, bits_remaining); fflush(stdout);
#endif

// Re-allocate KEK_authen_XMR_SHD array -- add 256 bytes.
      if ( (KEK_authen_XMR_SHD = (unsigned char *)realloc(KEK_authen_XMR_SHD, (current_XMR_SHD_num_bytes + SAP_ptr->num_required_PNDiffs/8) * 
         sizeof(unsigned char))) == NULL )
         { printf("ERROR: KEK_VerifierAuthentication(): Allocation for 'KEK_authen_XMR_SHD' failed!\n"); exit(EXIT_FAILURE); }

// Add the KEK_authen_XMR_SHD_chunk to the larger array to be sent to the device.
      for ( i = 0, j = current_XMR_SHD_num_bytes; i < SAP_ptr->num_required_PNDiffs/8; i++, j++ )
         KEK_authen_XMR_SHD[j] = SAP_ptr->KEK_authen_XMR_SHD_chunk[i];
      current_XMR_SHD_num_bytes += SAP_ptr->num_required_PNDiffs/8;

// Check if bits remaining to be encoded become zero. If so, we are done. If not, repeat the above.
      if ( bits_remaining == 0 )
         {
         target_attempts++;
         break;
...
// Sanity check -- something is wrong. Population SpreadFactors used and only one chip in database?
      if ( target_attempts == SAP_ptr->num_required_PNDiffs - 1 )
         { printf("ERROR: KEK_VerifierAuthentication(): Seed increment reached max value -- FAILED TO GET ENOUGH BITS!\n"); exit(EXIT_FAILURE); }
      target_attempts++;
      }

#ifdef DEBUG
printf("KEK_VerifierAuthentication(): Sending %d XMR_SHD_num_bytes to device\n", current_XMR_SHD_num_bytes); fflush(stdout);
#endif

// ------------------------------------------------------------------
// Send device the KEK_authen_XMR_SHD. Send the size first so the device can allocate storage.
   sprintf(current_XMR_SHD_num_bytes_str, "%d", current_XMR_SHD_num_bytes);
   if ( SockSendB((unsigned char *)current_XMR_SHD_num_bytes_str, strlen(current_XMR_SHD_num_bytes_str) + 1, device_socket_desc) < 0 )
      { printf("ERROR: KEK_VerifierAuthentication(): Send 'current_XMR_SHD_num_bytes_str' failed\n"); exit(EXIT_FAILURE); }

   if ( SockSendB(KEK_authen_XMR_SHD, current_XMR_SHD_num_bytes, device_socket_desc) < 0 )
      { printf("ERROR: KEK_VerifierAuthentication(): Send KEK_authen_XMR_SHD failed\n"); exit(EXIT_FAILURE); }

// Save design information and the XMR_SHD to the RunTime database if user requests it.
   if ( SAP_ptr->do_save_bitstrings_to_RT_DB == 1 )
      {
      pthread_mutex_lock(SAP_ptr->RT_DB_mutex_ptr);
      SaveDBBitstringInfo(max_string_len, SAP_ptr, KEK_authen_XMR_SHD, current_XMR_SHD_num_bytes, FUNC_VA);
      pthread_mutex_unlock(SAP_ptr->RT_DB_mutex_ptr);
      }

// ------------------------------------------------------------------
// Do the remaining iterations of calls to CommonCore -- this does ONLY part_B of CommonCore where consecutative sets of SpreadFactors are restored and 
// sent to the device. We did part_A above, i.e., the device already has the challenges, nonce exchange, etc. NOTE: target_attempts is FORCED to 0 when 
// 'do_part_A' is 1. NOTE: this function sets CommonCore parameters as follows: set_threshold_to_zero = 0 and send_SpreadFactor = 1 

// No need to recompute PCR or PBD SF.
   compute_PCR_PBD_SF = 0;
   int restore_store_SF = 0;

// Setting restore_store_SF to 0 and sending SpreadFactors_binary to NON-NULL will restore the existing SF (will NOT compute them) and will send them
// to the device.
   do_part_A = 0;
   target_attempts = 0;
   int done;
   int return_after_each_set = 0;
   target_attempts = GenChlngDeliverSpreadFactorsToDevice(max_string_len, SAP_ptr, do_part_A, target_attempts, RANDOM, device_socket_desc, &done, 
      return_after_each_set, compute_PCR_PBD_SF, restore_store_SF, &SpreadFactors_binary, current_function);

// Sanity check
   if ( done != 1 )
      { printf("ERROR: KEK_VerifierAuthentication(): done is NOT 1!\n"); exit(EXIT_FAILURE); }

// ------------------------------------------------------------------
   if ( SAP_ptr->DEBUG_FLAG == 1 )
      {
      printf("\tWaiting device's 'PASS/FAIL' signal\n");
      gettimeofday(&t0, 0);
      }
   if ( SockGetB((unsigned char *)request_str, max_string_len, device_socket_desc) != 5 )
      { printf("KEK_VerifierAuthentication(): Receive 'PASS/FAIL' request failed!\n"); exit(EXIT_FAILURE); }
   if ( strcmp(request_str, "FAIL") == 0 )
      fail_or_pass = 0;
   else if ( strcmp(request_str, "PASS") == 0 )
      fail_or_pass = 1;
   else
      { printf("KEK_VerifierAuthentication(): Did NOT receive 'PASS/FAIL' string => %s!\n", request_str); exit(EXIT_FAILURE); }
   if ( SAP_ptr->DEBUG_FLAG == 1 )
      { gettimeofday(&t1, 0); elapsed = (t1.tv_sec-t0.tv_sec)*1000000 + t1.tv_usec-t0.tv_usec; printf("\tElapsed %ld us\n\n", (long)elapsed); }

   if ( fail_or_pass == 0 )
      printf("\n\tKEK_VerifierAuthentication FAILED for chip %d\n\n", SAP_ptr->chip_num);
   else
      printf("\n\tKEK_VerifierAuthentication PASSED for chip %d\n\n", SAP_ptr->chip_num);
   fflush(stdout);
...
// Free up temporary storage. 
   if ( SpreadFactors_binary != NULL )
      free(SpreadFactors_binary); 
...
// ========================================================================================================
// ========================================================================================================
// The process of generating a session key uses the 'first strong bit' (FSB) algorithm.. Here, we start the 
// device to carry out KEK_Enroll, which uses the hardware version of KEK (FSB mode) to produce a session key 
// and XMR_SHD helper data. The XMR_SHD is then transmitted back here to the verifier, which uses the software 
// version of the FSB version of KEK to reproduce the key. NOTE: On the device, there is no KEK_SessionKeyGen() 
// routine because we just reuse KEK_Enroll with a flag.

int KEK_SessionKeyGen(int max_string_len, SRFAlgoParamsStruct *SAP_ptr, int device_socket_desc, int RANDOM)
   {
   int Threshold, current_function, compute_SpreadFactors, send_SpreadFactors, do_part_A, target_attempts; 
   int enroll_or_regen, SBS_num_bits, SHD_num_bytes, do_part_A_part_B_both, set_threshold_to_zero; 
   int num_key_bits, current_num_key_bits;

   unsigned char *XMR_SHD, XMR_SBS[SAP_ptr->num_required_PNDiffs/8];
   int received_XMR_SHD_num_bytes; 
   ...
   int do_minority_bit_flip_analysis, num_minority_bit_flips, true_mismatch_count;
...
// Here we know which device has authenticated and am doing session key gen. The server computes the PO/PCR/PBD SF and transmits to device. 
// Setting this to ONE ensures the device gets the modified PopOnly SF computed in the ComputePxxSpreadFactors routine for ANY mode.
// Set to 1 when the server needs to compute the SF COMPLETELY (device does NOT do it), in which case, the PopOnly SF MUST be updated to 
// flip the distribution on the device and then PCR are computed later (after the call to generate the Pop SF).
   SAP_ptr->do_PO_dist_flip = 1;
...
// 5_11_2021: Do minority bit flip report below when asserted.
   do_minority_bit_flip_analysis = 1;

   if ( (SAP_ptr->SE_target_num_key_bits % 8) != 0 )
      { printf("ERROR: KEK_SessionKeyGen(): Number of key bits requested must be a multiple of 8!\n"); exit(EXIT_FAILURE); }

#ifdef DEBUG
printf("KEK_SessionKeyGen(): Freeing SE_final_key!\n"); fflush(stdout);
#endif

// This should ALWAYS BE NULL. When I call this function, I assign a dedicated 'named' pointer to this key and NULL out this field.
// This is allocated by JoinBytePackedBitStrings below automatically. Just make sure it is NULL initially.
   if ( SAP_ptr->SE_final_key != NULL )
      free(SAP_ptr->SE_final_key);
   SAP_ptr->SE_final_key = NULL;

   printf("\t\t\t******************* SESSION KEY GEN BEGINS  ******************* \n\n"); fflush(stdout);

// First, ask the device to generate the session key, delivering challenges and then SpreadFactors in a loop as needed. This is identical to 
// device authentication at this point. A convenience function that calls CommonCore when do_part_A is 1 to generate challenges, 
// collect timing data from the database and exchange nonces with the device, and then it always does part_B of CommonCore where 
// consecutative sets of SpreadFactors are generated and sent to the device. NOTE: target_attempts is FORCED to 0 when 'do_part_A' is 1. 
// NOTE: this function sets CommonCore parameters as follows: set_threshold_to_zero = 0 and send_SpreadFactors = 1. 

// Optionally compute and send PCR/PBD SpreadFactors to device. 
   int restore_store_SF = 1;
   int compute_PCR_PBD_SF;
...
// Sanity check
   if ( done != 1 )
      { printf("ERROR: KEK_SessionKeyGen(): done is NOT 1!\n"); exit(EXIT_FAILURE); }

#ifdef DEBUG
printf("KEK_SessionKeyGen(): Getting XMR_SHD %d bytes from device!\n", target_attempts * SAP_ptr->num_required_PNDiffs/8); fflush(stdout);
#endif

   int XMR_num_bytes = target_attempts * SAP_ptr->num_required_PNDiffs/8;

// Expect to receive (target_attempts * SAP_ptr->num_required_PNDiffs/8) bytes of XMR_SHD from device. Allocate space.
   if ( (XMR_SHD = (unsigned char *)calloc(XMR_num_bytes, sizeof(unsigned char))) == NULL )
      { printf("ERROR: KEK_SessionKeyGen(): Allocation for 'XMR_SHD' failed!\n"); exit(EXIT_FAILURE); }

   if ( (received_XMR_SHD_num_bytes = SockGetB(XMR_SHD, XMR_num_bytes, device_socket_desc)) != XMR_num_bytes )
      { 
      printf("ERROR: KEK_SessionKeyGen(): Receive 'XMR_SHD' request failed -- expected %d!\n", XMR_num_bytes); 
      exit(EXIT_FAILURE); 
      }

#ifdef DEBUG
printf("KEK_SessionKeyGen(): About to generate Session key with XMR val %d!\n", SAP_ptr->XMR_val); fflush(stdout);
#endif

// ------------------------------------------------------------------
// ASSUME that the timing vals (PNR and PNF) have already been allocated and stored in the SAP fields based on a challenge and the 
// XOR_nonce has been set with the first call to CommonCore done in GenChlngDeliverSpreadFactorsToDevice. Since multiple calls to CommonCore 
// have been made to give updated SpreadFactors to the device for each iteration, we must call CommonCore here with do_part_A_part_B_both 
// set to 1, which will ONLY select parameters, set the LFSR_seed_high according to the number of target_attempts and compute the SpreadFactors 
// again that were sent to the device.
   current_num_key_bits = 0;

// Assume multiple iterations of KEK were needed to generate enough SE_target_num_key_bits helper data to create the session key.
   enroll_or_regen = 1;
   target_attempts = 0;
   while ( current_num_key_bits < SAP_ptr->SE_target_num_key_bits )
      {

// Restore the SpreadFactors
      int i;
      for ( i = 0; i < SAP_ptr->num_SF_words; i++ )
         {
         SAP_ptr->iSpreadFactors[i] = SpreadFactors_binary[target_attempts*SAP_ptr->num_SF_words + i]; 
         SAP_ptr->fSpreadFactors[i] = (float)SAP_ptr->iSpreadFactors[i]/(float)SAP_ptr->iSpreadFactorScaler;
         }

// Call CommonCore to reset the parameters (LFSR_seed_high) based on XOR_nonce and target_attempts. Reuse SpreadFactors but do NOT transmit them to device.
      compute_SpreadFactors = 0;
      send_SpreadFactors = 0;
...
// 10_26_2021: BUG: Forgot to do this here! Do SRF Engine operations in software. 
      DoSRFComp(max_string_len, SAP_ptr, 0);

// The KEK FSB Regen routine in common.c wants the raw bitstring (computed with Threshold set to 0). Run the SingleHelpBitGen algorithm with the 
// Threshold set to 0.
      Threshold = 0;
      SBS_num_bits = SingleHelpBitGen(SAP_ptr->num_required_PNDiffs, SAP_ptr->fPNDco, SAP_ptr->device_SBS, SAP_ptr->device_SHD, 
         &SHD_num_bytes, Threshold);
...
// Sanity check. With Threshold set to 0, the number of strong bits is the same size as the SHD.
      if ( SBS_num_bits != SAP_ptr->num_required_PNDiffs )
         { 
         printf("ERROR: KEK_SessionKeyGen(): Number of bits in raw bitstring must be %d -- found %d!\n", SAP_ptr->num_required_PNDiffs, SBS_num_bits); 
         exit(EXIT_FAILURE); 
         }

// Sanity check. The number of iterations here should NEVER exceed what the device did to generate the received_XMR_SHD_num_bytes.
      if ( target_attempts*SAP_ptr->num_required_PNDiffs/8 >= received_XMR_SHD_num_bytes )
         { printf("ERROR: KEK_SessionKeyGen(): SpreadFactors into XMR_SHD TOO LARGE!\n"); exit(EXIT_FAILURE); }

// RUN FSB algorithm in REGENERATION MODE: Generate a chunk of the session key and store it in XMR_SBS. Added the minority bit flip reporting. Note that 
// we DO NOT have the Session key from the device so we can NOT compute true_mismatch_count (NULL parameter to device Session key in call).
      num_minority_bit_flips = 0; 
      true_mismatch_count = 0;
      int FSB_or_SKE = 0;
      num_key_bits = KEK_FSB_SKE(SAP_ptr->num_required_PNDiffs, SAP_ptr->XMR_val, XMR_SHD + target_attempts * SAP_ptr->num_required_PNDiffs/8, 
         SAP_ptr->device_SBS, NULL, 0, XMR_SBS, enroll_or_regen, do_minority_bit_flip_analysis, &num_minority_bit_flips, current_num_key_bits,
         NULL, &true_mismatch_count, FSB_or_SKE, 0, 0);

// Print out a summary of the minority bit flips. NOTE: true_mismatch_count is NOT valid -- we do NOT have a KEK key to compare it to.
      if ( do_minority_bit_flip_analysis == 1 )
         { printf("\tMINORITY BIT FLIP SUMMARY: Num minority bit flips %d!\n", num_minority_bit_flips); fflush(stdout); }

// 3_14_2021: Copy key bytes into the allocated array. NOTE: This routine ALLOCATES space for the key as needed.
      current_num_key_bits = JoinBytePackedBitStrings(current_num_key_bits, &(SAP_ptr->SE_final_key), num_key_bits, XMR_SBS);

      target_attempts++;
      }

#ifdef DEBUG
printf("KEK_SessionKeyGen(): DONE -- number of bits generated %d!\n", current_num_key_bits); fflush(stdout);
#endif

// ------------------------------------------------------------------
// DEBUG: Dump out the values as hex. DO NOT PRINT OUT MORE BYTES THAN WAS ALLOCATED ABOVE.
   if ( SAP_ptr->DUMP_BITSTRINGS == 1 )
      {
      char header_str[max_string_len];
      sprintf(header_str, "\n\tFINAL SESSION KEY (Bits %d)\n", SAP_ptr->SE_target_num_key_bits);
      PrintHeaderAndHexVals(header_str, SAP_ptr->SE_target_num_key_bits/8, SAP_ptr->SE_final_key, 32);

      printf("\tSE Total number of iterations %d\n\n", target_attempts);
      }

// Save design information and the XMR_SHD and Session key to the RunTime database if user requests it.
   if ( SAP_ptr->do_save_bitstrings_to_RT_DB == 1 )
      {
      pthread_mutex_lock(SAP_ptr->RT_DB_mutex_ptr);
      SaveDBBitstringInfo(max_string_len, SAP_ptr, XMR_SHD, received_XMR_SHD_num_bytes, FUNC_SE);
      pthread_mutex_unlock(SAP_ptr->RT_DB_mutex_ptr);
      }

// ------------------------------------------------------------------
// Send a trial AES encryption using the verifier session key to the device for comparison. Send ONLY the number of bytes requested by the key size even
// if more than requested are generated. HERE WE ASSUME THE MAXIMUM AES block size is 256 bits.
   unsigned char trial_plain_text[AES_INPUT_NUM_BITS/8+1];
   unsigned char trial_encryption[AES_INPUT_NUM_BITS/8+1];

   int i;
   for ( i = 0; i < AES_INPUT_NUM_BITS/8+1; i++ )
      trial_encryption[i] = '\0';
   strcpy((char *)trial_plain_text, "HELLO WORLD!");

// 10_27_2021: Changed to 256-bit AES, CBC mode. Cyrus's wrapper.
   encrypt_256(SAP_ptr->SE_final_key, SAP_ptr->AES_IV, trial_plain_text, AES_INPUT_NUM_BITS/8, trial_encryption);

   if ( SockSendB(trial_encryption, AES_INPUT_NUM_BITS/8, device_socket_desc) < 0 )
      { printf("ERROR: KEK_SessionKeyGen(): Trial plain text send failed\n"); exit(EXIT_FAILURE); }
   ...
   if ( SockGetB((unsigned char *)request_str, max_string_len, device_socket_desc) != 5 )
      { printf("ERROR: KEK_SessionKeyGen(): Receive 'PASS/FAIL' request failed!\n"); exit(EXIT_FAILURE); }
   if ( strcmp(request_str, "FAIL") == 0 )
      fail_or_pass = 0;
   else if ( strcmp(request_str, "PASS") == 0 )
      fail_or_pass = 1;
   else
      { printf("KEK_SessionKeyGen(): Did NOT receive 'PASS/FAIL' string => %s!\n", request_str); exit(EXIT_FAILURE); }
   if ( SAP_ptr->DEBUG_FLAG == 1 )
      { gettimeofday(&t1, 0); elapsed = (t1.tv_sec-t0.tv_sec)*1000000 + t1.tv_usec-t0.tv_usec; printf("\tElapsed %ld us\n\n", (long)elapsed); }

   if ( fail_or_pass == 0 )
      {
      printf("\n\tSESSION KEY GENERATION FAILED for chip %d\n\n", SAP_ptr->chip_num);
      ...
      printf("\n\tSESSION KEY GENERATION PASSED for chip %d\n\n", SAP_ptr->chip_num);
   fflush(stdout);
...
// ========================================================================================================
// ========================================================================================================
// KEK provisioning. This is done once after manufacture.

void KEK_EnrollProvisioning(int max_string_len, SRFAlgoParamsStruct *SAP_ptr, int device_socket_desc, int RANDOM)
   {
   char request_str[max_string_len];
   int target_attempts;
...
// We send the device pure PopOnly SF and it modifies the PopOnly SF itself. Setting this to zero ensure the device gets 
// unmodified PopOnly SF computed in the ComputePxxSpreadFactors routine for ANY mode. When set to 1 when the server needs 
// to compute the SF COMPLETELY (device does NOT do it), in which case, the PopOnly SF MUST be updated to flip the 
// distribution on the device and then PCR are computed later (after the call to generate the Pop SF).
   SAP_ptr->do_PO_dist_flip = 0;
...
// A convenience function that calls CommonCore when do_part_A is 1 to generate challenges, collect timing data from the database 
// and exchange nonces, and then always does part_B of CommonCore where consecutative sets of SpreadFactors are generated and 
// sent to the device. NOTE: target_attempts is FORCED to 0 when 'do_part_A' is 1. NOTE: this function sets CommonCore parameters 
// as follows: set_threshold_to_zero = 0 and send_SpreadFactors = 1.

// Optionally compute and send PopSF to device. Device computes it's own PCR/PBD component.
   int compute_PCR_PBD_SF = 0;
   int restore_store_SF = 0;
   ...
   int done;
   int return_after_each_set = 0;
   target_attempts = GenChlngDeliverSpreadFactorsToDevice(max_string_len, SAP_ptr, do_part_A, target_attempts, RANDOM, 
      device_socket_desc, &done, return_after_each_set, compute_PCR_PBD_SF, restore_store_SF, NULL, current_function);

// Sanity check
   if ( done != 1 )
      { printf("ERROR: KEK_EnrollProvisioning(): done is NOT 1!\n"); exit(EXIT_FAILURE); }

// DATABASE VERSION ONLY -- do NOT free these for the FILE VERSION.
   if ( SAP_ptr->database_NAT != NULL )
      {
      FreeVectorsAndMasks(&(SAP_ptr->num_vecs), &(SAP_ptr->num_rise_vecs), &(SAP_ptr->first_vecs_b), &(SAP_ptr->second_vecs_b), 
         &(SAP_ptr->masks_b));
      FreeAllTimingValsForChallenge(&(SAP_ptr->num_chips), &(SAP_ptr->PNR), &(SAP_ptr->PNF));
      }

// Get SHD from device.
//   device_SHD_num_bytes = GetDeviceSHD(max_string_len, max_PNDiffs/8, device_socket_desc, device_SHD, DEBUG_FLAG);

   if ( SAP_ptr->DEBUG_FLAG == 1 )
      {
      printf("\tWaiting device's 'ACK' signal\n");
      gettimeofday(&t0, 0);
      }

   if ( SockGetB((unsigned char *)request_str, MAX_STRING_LEN, device_socket_desc) != 4 )
      { printf("ERROR: KEK_EnrollProvisioning(): Receive 'ACK' request failed!\n"); exit(EXIT_FAILURE); }
   if ( strcmp(request_str, "ACK") != 0 )
      { printf("ERROR: KEK_EnrollProvisioning(): Did NOT receive 'ACK' string => %s!\n", request_str); exit(EXIT_FAILURE); }

// Send 'ACK back to device
   strcpy(request_str, "ACK");
   if ( SockSendB((unsigned char *)request_str, strlen(request_str) + 1, device_socket_desc) < 0 )
      { printf("ERROR: KEK_EnrollProvisioning(): Failed to send '%s' to device!\n", request_str); exit(EXIT_FAILURE); }

   if ( SAP_ptr->DEBUG_FLAG == 1 )
      { gettimeofday(&t1, 0); elapsed = (t1.tv_sec-t0.tv_sec)*1000000 + t1.tv_usec-t0.tv_usec; printf("\tElapsed KEK_ENROLLPROVISIONING %ld us\n\n", (long)elapsed); }
   fflush(stdout);
...
// =========================================
// Device Authentication
   if ( KEK_ClientServerAuthen(max_string_len, SAP_ptr, client_socket_desc, RANDOM) == 0 )
      return 0;

// =========================================
// Verifier authentication. Only have the SHD bitstring when VerifierAuthentication is carried out so not saving anything right now. We would need
// to transmit the DHD bitstring back from the device.
   gettimeofday(&t2, 0);
   if ( KEK_VerifierAuthentication(max_string_len, SAP_ptr, client_socket_desc, RANDOM) == 0 )
      return 0;

#ifdef DEBUG
printf("KEK_ClientServerAuthenKeyGen(): About to FREE vectors and timing data\n"); fflush(stdout);
#endif
...
#ifdef DEBUG
printf("KEK_ClientServerAuthenKeyGen(): DONE to vectors and timing data\n"); fflush(stdout);
#endif

// =========================================
// Session key generation. Key is dynamically allocated and stored in SAP structure.
   if ( gen_session_key == 1 )
      {
      int fail_or_pass;

      gettimeofday(&t2, 0);
      fail_or_pass = KEK_SessionKeyGen(max_string_len, SAP_ptr, client_socket_desc, RANDOM);
...
// DATABASE VERSION ONLY -- do NOT free these for the FILE VERSION.
      ...
      gettimeofday(&t1, 0); elapsed = (t1.tv_sec-t2.tv_sec)*1000000 + t1.tv_usec-t2.tv_usec; printf("\tElapsed: SESSION ENCRYPTION %ld us\n\n", (long)elapsed);
      if ( fail_or_pass == 0 )
         return 0;
      }
...
// ========================================================================================================
// ========================================================================================================
// KEK enrollment after provisioning. NOTE: We need to free the SE_final_key once we are done with it in
// the caller.
...
// Here, we must first authenticate and THEN generate new KEK challenge information.
   gen_session_key = 1;
   if ( KEK_ClientServerAuthenKeyGen(max_string_len, SAP_ptr, device_socket_desc, RANDOM, gen_session_key) == 0 )
      { 
      printf("ERROR: KEK_EnrollInField(): Failed to authenticate or generate a session key with device!\n"); 
      fflush(stdout);
//      exit(EXIT_FAILURE); 
      }

// Call the function responsible for generating a new KEK challenge for this device.
   KEK_EnrollProvisioning(max_string_len, SAP_ptr, device_socket_desc, RANDOM);
