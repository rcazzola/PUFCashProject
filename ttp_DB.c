...
printf("AliceWithdrawal(): Getting chip_num from Alice so we can fetch an AT for Alice from the Bank!\n"); fflush(stdout); 
#ifdef DEBUG
#endif
...
printf("AliceWithdrawal(): TTP getting AT for Alice's chip_num %d!\n", chip_num); fflush(stdout); 
#ifdef DEBUG
#endif
...
printf("AliceWithdrawal(): TTP carrying out ZeroTrust protocol with Alice (chip_num %d)!\n", chip_num); fflush(stdout); 
#ifdef DEBUG
#endif
...
printf("AliceWithdrawal(): Exchange ID's completed successfully with Alice's chip_num %d!\n", Alice_chip_num); fflush(stdout);
#ifdef DEBUG
#endif
...
// ZeroTrust protocol. 
   SHP_ptr->ZHK_A_num_bytes = SE_TARGET_NUM_KEY_BITS/8;
